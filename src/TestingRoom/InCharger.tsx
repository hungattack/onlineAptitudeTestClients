...
const InCharger: React.FC<{
...
}> = ({ roomData, catePart, setCatePart, setStart }) => {
    ...
    const { status, startTime, canProcess, id_room, calculate } = useSelector(
        ...
        if (startTime.some((s) => s.id === catePart && !s.finish)) {
            ...
            if (ok) {
                ...
                if (roomData.Cates.$values[index + 1]) {
                    ...
                    setIndex({ id: roomData.Cates.$values[index + 1].Questions.$values[0].Id, index: 0 });
                } else {
    ...
    return (
        <Div display="block" css="height: 100%; color: #fff; position: relative;">
            <Div
                css={`
                    height: 35px;
                    border-radius: 5px;
