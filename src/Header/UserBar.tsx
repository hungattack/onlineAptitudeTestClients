...
const UserBar = ({ setShow }: { setShow: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    console.log(user?.gender, avatar, 'avatar');
    ...
    return (
        <>
            {user ? (
                ...
                <div style={{ color: '#f1f1f1', width: '50%' }} className={styles.gridDot}>
                    <div className={styles.gridDotShow} onClick={() => setShow((pre) => !pre)}>
                        <GridDotI />
                    </div>
                </div>
            )}
